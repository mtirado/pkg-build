...
case "$PKGARCHIVE" in
	...
	#	patch -p1 < $_PKG_DIR/1-avoid-gets-error.patch
	#	./configure 			\
	#		--prefix=$PKGROOT
	#;;
	glibc-*)
		# JOBS=1 because write stdout error make bug
		#JOBS=1
		...
		mkdir -vp "$BDIR"
		if [ "$PKGNOCONF" == "" ]; then
			# geeze louise, there's a libstdcxx and libc macro conflict?
			patch -p1 < "$_PKG_DIR/glibc-2.26-gcc-lt6-signbit.patch"
